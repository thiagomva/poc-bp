...
exports.makeV1GaiaAuthToken = makeV1GaiaAuthToken;
...
function makeV1GaiaAuthToken(hubInfo, signerKeyHex, hubUrl, associationToken, scopes) {
 ...
 */
function connectToGaiaHub(gaiaHubUrl, challengeSignerHex, associationToken, scopes) {
  if (!associationToken) {
  ...
  return fetch(gaiaHubUrl + '/hub_info').then(function (response) {
    ...
    var token = makeV1GaiaAuthToken(hubInfo, challengeSignerHex, gaiaHubUrl, associationToken, scopes);
